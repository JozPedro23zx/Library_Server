...
class SendBookServices{
    ...
    async randomRecommend(genresArray, atualLibrary){
        ...
        if(genresArray.length != 0){
            ...
            recommend = await Book.findAll({where:{title: allTitles, libraryId: atualLibrary}, limit: 4, order: Sequelize.literal('rand()')})
            return recommend
        }
        recommend = await Book.findAll({where:{libraryId: atualLibrary}, limit: 4, order: Sequelize.literal('rand()')})    
        return recommend
